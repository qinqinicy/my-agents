<template>
  <div class="card-wrapper flex flex-col py-[14px] px-[16px]">
    <div class="flex mb-[10px]">
      ...
      <source-card
        :lable="item.order_status_name"
        class="ml-[8px]"
      ></source-card>
    </div>
    <div class="flex mb-[4px]">
      <span class="label">事件编码：</span>
      <span class="value flex-1">{{ item.eventCode }}</span>
    </div>
    <div class="flex mb-[4px]">
      <span class="label">发生时间：</span>
      <span class="value flex-1">{{ item.reportTime }}</span>
    </div>
    <div class="flex mb-[4px]">
      <span class="label">发生地址：</span>
      <span class="value flex">{{ item.happenAddress }}</span>
    </div>
  </div>
</template>
<script setup>
